...
\end{equation}
computes the joint torques incorporating a damping $ D(q) $, a stiffness $ \frac{\partial V_d(q)}{\partial q} $ and a gravitational $ g(q) $ term. $ V_d $ and $ D $ are formulated in operational space first, so the impedance relation can be re-formulated in operational space:    
\begin{equation}
D\Delta\dot{x} + \dfrac{\partial V_d(\Delta x)}{\partial x} = h
\end{equation}
This control law may generate either a reference trajectory or command  end-effector forces.\\
The stiffness contribution divides in a object translation/rotation and a object-manipulator connection part. The virtual object frame is connected to a desired pose via a 6-DOF spring. Translational and rotational stiffness potentials are: 
\begin{subequations}
\begin{align}
