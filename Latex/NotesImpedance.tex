...
\subsection{Sieber, Musi\'{c}, Hirche, "Formation-based robot team controlled by a single human", 2015 }
Yet another approach for controlling internal forces is \textit{formation control scheme} presented in this work. It uses \textit{Artificial Potential Fields} to define a realize desired distance $ d_{ij} $ between to end-effectors $ i $ and $ j $:
...
\begin{multline}
u_{i,k} = - \sum_{\substack{j\in N_i}}\delta_{ij}\dfrac{\partial V_{ij,k}(\vert x_{i,k}-x_{j,k}\vert)}{\partial x_i} = \\
...
The potential (\ref{SieberMusicArtificialPotential}) can be rewritten in vector notation:
\begin{equation}
V_{ij}(\vert x_{i}-x_{j}\vert) = \dfrac{1}{2}(\vert x_{i}-x_{j}\vert - d_{ij})^{T}(\vert x_{i}-x_{j}\vert - d_{ij})
\end{equation}
Derivation by $ x_i $ leads to:
The \textit{virtual linkage} model defines internal force as the differential force acting along a virtual linkage, i.e.the connection line of two grasp points. Therefore the relation between internal force $ t $ and applied force $ f $ is $ f = Et $. Where $ E $ contains the unit vectors $ e_{ij} $ along the link from $ x_i $ to $ x_j $, i.e. $ e_{ij}  = \dfrac{x_j-x_i}{\vert x_j-x_i \vert}  $. $ E $ can be derived by calculating $ \dfrac{\partial\Delta x}{\partial x} $ and normalizing the resulting vectors. Here $ \Delta x_i = \vert x_j - x_k \vert $ stands for all possible connections of end-effectors.
\section{Conclusions}
