...
Where $T_i^j$ is simply described as the twist of $\Psi_i$ with respect to $\Psi_j$, naturally given in reference frame $\Psi_j$ (and therefore not noted as $T_i^{j,j}$). \\
...
Here the mapping is in the opposite direction, from $\Psi_j$ to $\Psi_i$, what is a consequence of the fact that wrenches are duals to twists. Again there different representations of wrenches: \begin{itemize}
...
In cooperative manipulation we often deal with heavy objects, it is thus inevitable to model gravity of the virtual object. A mass in the gravitational field has a potential energy and can be modelled with a spring connecting the body and an inertial frame associated with the ground. This spring can be formulated as a PHS in the same manner than in the previous sections
\begin{eqnarray}
	\dot{H}_b^0 = T_b^{0,0}H_b^0\\
	W_b^{0,0} = \frac{\partial V_{g}}{\partial H_b^0}(H_b^0)^T
\end{eqnarray}
The combined PHS representation of an inertia is \cite{Stramigioli_01}
\begin{eqnarray} \label{EQ:PHSinertia}
\begin{pmatrix}\dot{H}_b^0 \\ \dot{P_b^b}\end{pmatrix} =
\begin{pmatrix}0 & Ad_{H_b^0} H_o^b \\
-(H_o^b)^T Ad_{H_b^0}^T & C_b\end{pmatrix}
\begin{pmatrix}\frac{\partial V_{kg}}{\partial H_b^0} \\ \frac{\partial V_{kg}}{\partial P_b^b}\end{pmatrix} + 
\begin{pmatrix}0 \\ I_6\end{pmatrix} W_b^b \\
...
\begin{eqnarray}\label{EQ:variablerestlengthspring}
	\dot{H}_{v(i)}^i = \begin{pmatrix}1 & Ad_{H_b^i}\end{pmatrix} \begin{pmatrix}T_b^{i} \\ T_{v(i)}^{b}\end{pmatrix}H_{v(i)}^i\\
	\begin{pmatrix}W_b^{i,i} \\ W_{v(i)}^{b,b}\end{pmatrix}  = \begin{pmatrix}1 \\ Ad_{H_b^i}^T\end{pmatrix} \frac{\partial V_{v(i),i}}{\partial H_{v(i)}^i}(H_{v(i)}^i)^T
\end{eqnarray}
