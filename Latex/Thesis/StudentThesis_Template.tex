...
\end{itemize}
Note that the first representation is mainly used for bodies, the wrench is exerted \emph{on} the body. The second is useful for springs, the wrench is exerted on the spring \emph{by} the body: $W_i^k = - W_i^{k,j}$. Also it is clear from Newton's third law (action - reaction) that $W_i^{k,j} = - W_j^{k,i}$.
...
\[T_v^b = (Ad_{H_0^b} \;\; -Ad_{H_0^b}) \begin{pmatrix} T_v^0 \\ T_b^0 \end{pmatrix}  \]
...
\[ \begin{pmatrix}W_v^{0} \\ W_b^{0}\end{pmatrix}  = 
\begin{pmatrix}Ad_{H_0^b}^T \\ -Ad_{H_0^b}^T \end{pmatrix} W_v^{b} \]
We can write the resulting PHS
\begin{eqnarray}
	\dot{H}_v^b = (Ad_{H_0^b} \;\; -Ad_{H_0^b}) \begin{pmatrix} T_v^0 \\ T_b^0 \end{pmatrix} H_v^b \\
	\begin{pmatrix}W_v^{0} \\ W_b^{0}\end{pmatrix}  = 
	\begin{pmatrix}-Ad_{H_0^b}^T \\ Ad_{H_0^b}^T \end{pmatrix}
	\frac{\partial V_{v,b}}{\partial H_v^b} (H_v^b)^T
\end{eqnarray} 
This is done with the manipulator springs in the same manner. Note that the twists and wrenches due to rest-length changes are not brought to the inertial frame, since they represent a distinct power port. As a result we can combine all twists and wrenches in vectors
