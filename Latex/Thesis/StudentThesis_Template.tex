...
A^T(q)\dot{q}=0
\end{equation}
We call $A(q)$ the \emph{constraint} matrix. The associated internal forces are given by $A(q)\lambda$, the \emph{Lagrange} multipliers $\Lambda$ are uniquely defined if the constraints are satisfied. In this case the constraint forces do not influence the energy of the system since $\lambda^T (A^T(q)\dot{q}) = 0$. Consequently the port-Hamiltonian system becomes a mixed set of differential and algebraic equations (DAE).
...
\end{equation}
This is an analytic expression for the constrained forces, $\lambda$ is re-inserted into eq. (\ref{EQ:PHSpconstrained}) and we obtain a set of ODEs in input-output form. Note that the forces $\lambda$ keep the velocities $A(q)\dot{q}$, i.e. if the constraint $A(q)\dot{q}=0$ was satisfied in the beginning it will remain satisfied.\\
...
\end{aligned}
\end{eqnarray}
Conclusively we have obtained two different representations of constrained systems in input-output form. The first one allows explicitly to calculate the constraint forces necessary to comply with the constraints. The second one restricts the possible motion and results in a system of reduced \emph{degrees of freedom}. We see in the remainder that the first can be exploited for control issues. The latter gives a system model, useful when describing the response to an external wrench acting on the common object.


\subsubsection{Constraints for 6D-motion}  

When it comes to constraints in six dimensions of freedom it is necessary to revisit the concept of twists used throughout this thesis. Recall the definition of a twist from eq. (\ref{EQ:righttranslation}) being
\begin{eqnarray}
\begin{aligned}
...
\end{aligned}
\end{eqnarray}
This immediately leads to $T_i^0 = T_b^0$. For a system if $i=1...N$ manipulators we can thus write the constraining condition
\begin{equation}
...
\end{pmatrix}
\end{equation}
We start applying the first method presented int the previous subsection by differentiating the constraints.
The constraint matrix $A$ is not time dependent, eq. (\ref{EQ:constraintderivative}) simplifies to
\begin{equation}
0 = \frac{d}{dt}A^TT = A^T\dot{T} = A^TM^{-1}\dot{P}
\end{equation}
First of all we look at the twist's time-derivative and obtain an equivalent formulation of constraint dynamics as for a classic approach e.g. \cite{Erhart_16}.
Consider the time-derivative of the constraint for the rigidly connected bodies $b$ and $i$.
\begin{eqnarray}
\begin{aligned}
...
\end{aligned}
\end{eqnarray}
The chosen twist representation fulfils the second-order dynamic requirements including centripetal terms.\\
Towards solving the set of DAEs, we insert the port-Hamiltonian representations into the constraint equation
\begin{eqnarray}
\begin{aligned}
0 = A^T M^{-1} \dot{P} = A^T M^{-1}(W+CT+A\lambda)
\end{aligned}
\end{eqnarray}
and solve for $\lambda$
\begin{equation}
\lambda = -(A^TM^{-1}A)^{-1}A^TM^{-1}(W+CT)
\end{equation}
Consider again the example of two rigidly connected bodies $b$ and $i$. Let us first examine the part $A^TM^{-1}CT$, using eq. (\ref{EQ:centripetaldetail}) and assuming that the bodies share the orientation $R_0^b = R_0^i$, we obtain
\begin{eqnarray}
\begin{aligned}
A^TM^{-1}\begin{pmatrix}
C_b T_b^{b,0} \\ C_i T_i^{i,0}
\end{pmatrix} &= \begin{pmatrix}
0 \\ \tilde{\omega}_b^{b,0} v_b^{b,0} - \tilde{\omega}_i^{i,0} v_i^{i,0} 
\end{pmatrix} = \\ &=
\begin{pmatrix}0 \\
\tilde{\omega}_b^{b,0} \tilde{p}_b^0 R_0^b \omega_b^0 + \tilde{\omega}_b^{b,0} R_0^b v_b^0 - \tilde{\omega}_b^{b,0} \tilde{p}_i^0 R_0^b \omega_b^0 - \tilde{\omega}_b^{b,0} R_0^b v_i^0
\end{pmatrix} = \\ &=
\begin{pmatrix}0 \\
\tilde{\omega}_b^{b,0} (\tilde{p}_b^0-\tilde{p}_i^0) \omega_b^{b,0} \end{pmatrix} = \begin{pmatrix}0 \\
\tilde{\omega}_b^{b,0} \tilde{p}_b^i \omega_b^{b,0}
\end{pmatrix}
\end{aligned}
\end{eqnarray}
Once again we have obtained a term for the centripetal acceleration, the resulting $\lambda$ is
\begin{equation}
\lambda = \left[A^TM^{-1}A)^{-1}(M_b^{-1}W_b^b -M_i^{-1}W_i^i  + \begin{pmatrix}0 \\
\omega_b^{b,0} \times (\omega_b^{b,0} \times {p}_b^i)
\end{pmatrix}\right]
\end{equation}
...
%Note that the wrench representation is based on a torque $\tau$ taking effect around a screw axis and a linear force $f$ acting along this axis. (citation would be good)
%\begin{equation}
%W = \begin{pmatrix}
