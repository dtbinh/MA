...
\begin{equation}
M_{o,d} (\ddot{x}_d - \ddot{x}) + D_o (\dot{x}_d - \dot{x}) + K(x_d,x) = h_{ext} 
\end{equation}
The dynamics of the virtual object is than given as:
\begin{equation}
M_{o,d}\ddot{x} = -h_{ext} + h_o^{\Sigma} = -h_{ext} + M_{o,d}\ddot{x}_d + D_o (\dot{x}_d - \dot{x}) + K(x_d,x)
\end{equation}
The resulting impedance control $ h_o^{\Sigma} $ force is than mapped to the manipulators using a generalized inverse of the grasp matrix:
\begin{equation}
h^{\Sigma} = G^\dagger h_o^{\Sigma}
\end{equation}
What is omitted in this strategy are the actual object dynamics. 
