...
We can simplify equation \ref{EQ:generalPHS} by setting $x = (q^T,p^T)^T$ and introduce a general port-Hamiltonian system of the form
\begin{eqnarray}
\begin{aligned}
...
\section{A model-based controller for cooperative manipulation}\label{S:modelbasedcontrol}
...
Towards a conclusion on stability of the overall system, it is necessary to take the environmental conditions into consideration. Letting a human explicitly control a cooperative robot-team rises the question of her/his dynamic behaviour and the impact on the system. Furthermore the environment, the robotic system is interacting with, can change unexpectedly. Stability considerations based on certain environment models may thus lose their basis. Preserving stability is especially crucial when the human operator is on-site to ensure her/his safety. The objective of this chapter is to introduce an energy tank with an energy controller to maintain a safe level of energy in the system, regardless of the dynamics of human and environment. The energy-based controller from chapter \ref{C:pHS modelling} integrates seamlessly with the tank concept.
\section{Passivity and unknown environments} 
Passivity is a key concept when dealing with unknown environments. It is well known that passive systems are stable with any system, regardless its structure or complexity, that can provide only a bounded amount of energy. On the other hand it is shown in \cite{Stramigioli_15}, that for a non-passive system there always exists an environment that destabilizes the overall behaviour. It is often argued that the human arm is not distinguishable from a mechanical impedance, this stems from experiments with a human manipulating a controlled impedance \cite{Hogan_89}. The robotic device applies a force to the human and s/he responds with motion, therefore the human-machine interface is on an energetic level (the product of force and velocity is mechanical power). In recent years new input methods appeared (hand tracking, gesture control). Here the interaction between human and robotic system is an exchange of information. Feedback given through tactile or visual information does not effectively restrict the operator's motion allowing him to issue nearly arbitrary commands. In order to obtain passive human-guided robotic system we propose to assign the operator with an energy budget at disposal to issue commands. This allows achieve stability with any environment the robotic system is interacting with, as long as its energy supply is bounded. Stability is given without requiring certain models or assumptions for human and environment. Furthermore the energy stored in the robotic set-up is controlled, this effectively limits the possible impact on the environment and therefore contributes to the safety if a human is on-site.       
\begin{figure}[b!]
	\centering
	\small
...
The motivation for the use of an \emph{energy tank} is to energetically decouple the human operator from the robotic system, i.e. the operator is no longer able to supply energy to the system. This is a consequence of the chosen input method, which based on information rather than on energy exchange, as discussed above. The energy necessary for driving the robotic system comes from the tank, the operator only controls the energetic flow.\\
The total energy stored either in the tank or in the model based controller is always constant, for the moment we assume there is no energy exchange with the environment. Thus we require the combined system to be lossless. The dampers in the model-based controller extract energy, we seek to re-gain the dissipated energy and put it back into the tank.\\ 
The energy tank is a virtual storage element defined with a Hamiltonian energy function. Let $x_t$ denote the (scalar) energy state and we have a simple, positive definite function $T(x_t) = \frac{1}{2}x_t^2$. The dynamic equations of the tank are
\begin{eqnarray}
\begin{aligned}
\dot{x_t} &= u_t \\
...
\end{aligned}
\end{eqnarray}
Here $u_t,y_t$ denote the input and output respectively. Towards the application of the energy tank we consider an example port-Hamiltonian system of the form
...
\end{aligned}
\end{eqnarray}
This standard system is chosen to keep the derivation simple and can be replaced with the controllers from section \ref{S:modelbasedcontrol}.
...
\end{aligned}
\end{eqnarray}
where $u_k,y_k$ is the input-output pair. Towards pairing the variable stiffness spring with the energy tank we can express $\kappa$ as a function of the tank level. By using the tank power flow equation (\ref{EQ:tankpower}) we obtain
\begin{equation}
\dot{\kappa} = \begin{cases}
0 & \text{if } \, T(x_t)\geq T_{th} \\
\frac{k}{T_{th}}\dot{T}(x_t) = \frac{k}{T_{th}} \dot{x}_t \frac{\partial \mathcal{V}_{\kappa}}{\partial x_t} & \text{if }  \, T(x_t) < T_{th}
\end{cases}
\end{equation}
...
\begin{cases}
0 & \text{if } \, T(x_t)\geq T_{th} \\
\frac{\partial \mathcal{V}_{\kappa}}{\partial \kappa}\frac{k}{T_{th}} \dot{x}_t \frac{\partial \mathcal{V_{\kappa}}}{\partial x_t} & \text{if }  \, T(x_t) < T_{th}
\end{cases}
\end{equation}
The power exchanged by the energy tank is of the form $\dot{T}(x_t) = \frac{\mathcal{V}_{\kappa}}{\partial x_t}u_t$, therefore $\frac{\partial \mathcal{V}_{\kappa}}{\partial \kappa}\frac{k}{T_{th}} \dot{x}_t$ is an input for the energy tank. Using these assignments we can write the port-Hamiltonian representation of variable stiffness spring and energy tank. For simplicity that we assume that the variable stiffness spring is part of the general system of equation (\ref{EQ:generalPHS})
\begin{equation}
\begin{pmatrix}
\dot{x} \\ \dot{x}_t \\ \dot{\kappa}
\end{pmatrix} =
\left[
\begin{pmatrix}
J(x) & \frac{\alpha w}{x_t} & 0\\ -\frac{\alpha w^T}{x_t} & 0 & -\frac{k}{T_{th}}\dot{x}_t \\ 0 & \frac{k}{T_{th}}\dot{x}_t & 0\end{pmatrix}
- 
\begin{pmatrix}
R(x) & 0 & 0\\ -\frac{1}{x_t}\frac{\partial \mathcal{V}^T}{\partial x}R(x) & 0 & 0 \\ 0 & 0 & 0
\end{pmatrix}
\right]
\begin{pmatrix}
\frac{\partial \mathcal{V}_{\kappa}}{\partial x} \\
