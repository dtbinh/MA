...
%DIF ADD StudentThesis_Template.tex   Sun Jun 12 20:42:42 2016
%DIF 2c2
%DIF < \usepackage{psfrag,pstricks,amsbsy,graphics,float,amsmath,amssymb,epstopdf}
%DIF -------
\usepackage{psfrag,pstricks,amsbsy,graphics,float,amsmath,amssymb,epstopdf,amsthm} %DIF > 
%DIF -------
...
%DIF < \psfrag{q1}[Bl][Bl]{\small $\alpha$}
			%DIFDELCMD < \includegraphics[width=0.4\textwidth]{general_setup.eps}
%DIFDELCMD < 			%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{We call }\DIFdelend \DIFaddbegin \DIFadd{where }\DIFaddend $A(\f{q}) \in \mathbb{R}^{k \times l}$ \DIFaddbegin \DIFadd{is }\DIFaddend the \emph{constraint} matrix \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $l$ \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{being }\DIFaddend the number of independent kinematic constraints. The derivation \DIFaddbegin \DIFadd{of the constrained Hamiltonian equations }\DIFaddend starts from the Euler-Lagrange equations of constrained motion \cite{duindam2009geoplexbook}
\begin{eqnarray}
\begin{aligned}
\frac{d}{dt}\left(\DIFdelbegin %DIFDELCMD < \dfrac{\partial \mathcal{L}}{\dot{\f{q}}}%%%
\DIFdelend \DIFaddbegin \dfrac{\partial \mathcal{L}}{\partial \dot{\f{q}}}\DIFaddend \right) - \frac{\partial \mathcal{L}}{\partial \f{q}} &= G(\f{q})\f{u} + A(\f{q})\boldsymbol{\lambda}
\\
A^T(\f{q})\dot{\f{q}} &= 0
\end{aligned}
\end{eqnarray}
...
\end{equation}
Here \DIFdelbegin \DIFdel{we assume that }\DIFdelend the inertias are \DIFaddbegin \DIFadd{assumed to be }\DIFaddend not configuration dependent, i.e. time-invariant. \DIFdelbegin \DIFdel{Now we insert the }\DIFdelend \DIFaddbegin \DIFadd{A real robot has multiple joints and link, its effective inertia is thus configuration dependent. Feedback-linearisation techniques \mbox{%DIFAUXCMD
\cite{Ott_04} }%DIFAUXCMD
compensate for the internal dynamic in favour of shaping a desired inertia. The assumption of a non-configuration dependent manipulation inertia is reasonable, if a perfect feedback-linearisation is implemented on the robot. Then the insertion of the }\DIFaddend port-Hamiltonian system equation for $\dot{P}$ \DIFdelbegin \DIFdel{and obtain
...
\DIFaddbegin \begin{figure}
	...
	\caption[General set-up of operator, controller and robots]{\DIFaddFL{The model-based controller virtually links human and robots}}
...
The scheme is object-centred, i.e. the user controls the object \DIFdelbegin \emph{\DIFdel{explicitly}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{directly }\DIFaddend and the controller generates appropriate commands for the robotic system. \DIFdelbegin \DIFdel{Explicit }\DIFdelend \DIFaddbegin \DIFadd{Direct }\DIFaddend here refers to a virtual coupling between the user (i.e. the reference trajectory given by the user) and the virtual object, established by a spring and a damper.\\
...
\begin{eqnarray}
\DIFdelbegin %DIFDELCMD < \resizebox{1\hsize}{!}{$
%DIFDELCMD < \begin{aligned}
%DIFDELCMD < \begin{pmatrix}\dot{H}_b^0 \\ \dot{H}_b^v \\  \dot{P_b^b} \\ \dot{H}_{b(i)}^i \\ \dot{P}_i^i\end{pmatrix}
%DIFDELCMD <  &=
%DIFDELCMD < \begin{pmatrix} 0 & 0 & H_b^0 & 0 & 0\\ 0 & 0 & H_b^v & 0 & 0 \\
%DIFDELCMD < - {H_b^0}^T & -{H_b^v}^T & C_b-D_b & -Ad_{H_b^{b(i)}}^T {H_{b(i)}^i}^T & -Ad_{H_b^i}^T D_i \\
%DIFDELCMD < 0 & 0 & H_{b(i)}^i Ad_{H_b^{b(i)}} & 0 & -H_{b(i)}^i Ad_{H_i^{b(i)}} \\ 0 & 0 & D_i Ad_{H_b^i} & Ad_{H_i^{b(i)}}^T {H_{b(i)}^i}^T & C_i - D_i\end{pmatrix}
...
%DIFDELCMD < \frac{\partial \mathcal{H}}{\partial P_{i}^i}\end{pmatrix} \\
%DIFDELCMD < &+
%DIFDELCMD < \begin{pmatrix}
%DIFDELCMD < 0 & 0 \\
%DIFDELCMD < -H_b^v Ad_{H_0^b} & 0 \\
%DIFDELCMD < D_b Ad_{H_0^b} & 0 \\
%DIFDELCMD < 0 & 0 \\
%DIFDELCMD < 0 & I_6
%DIFDELCMD < \end{pmatrix}
%DIFDELCMD < \begin{pmatrix}
%DIFDELCMD < T_v^0 \\ W_i^i
%DIFDELCMD < \end{pmatrix}
%DIFDELCMD < \\
%DIFDELCMD < \begin{pmatrix}
%DIFDELCMD < W_v^{0,0} \\ T_i^{i,0}
%DIFDELCMD < \end{pmatrix}
%DIFDELCMD < &=
%DIFDELCMD < \begin{pmatrix}
%DIFDELCMD < 0 & -Ad_{H_0^b}^T  {H_b^v}^T & Ad_{H_0^b}^T D_b^T & 0 & 0 \\
%DIFDELCMD < 0 & 0 & 0 & 0 & I_6
%DIFDELCMD < \end{pmatrix}
%DIFDELCMD < \begin{pmatrix}\frac{\partial \mathcal{H}}{\partial H_b^0}\\ \frac{\partial \mathcal{H}}{\partial H_b^v} \\ \frac{\partial \mathcal{H}}{\partial P_b^b} \\ \frac{\partial \mathcal{H}}{\partial H_{b(i)}^i} \\ 
%DIFDELCMD < \frac{\partial \mathcal{H}}{\partial P_{i}^i}\end{pmatrix}
...
%DIFDELCMD < $}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \resizebox{1\hsize}{!}{$
\begin{aligned}
...
Note that the  \DIFdelbegin \DIFdel{output to the robot side is a twist, the control scheme thus generates }\DIFdelend \DIFaddbegin \DIFadd{control output generated by this controller is twist, i.e. }\DIFaddend a compliant reference trajectory for the robots to follow. Similar to \cite{Caccavale_08} we require a local \DIFaddbegin \DIFadd{(on joint level) }\DIFaddend underlying force control layer to operate the robots.
\subsubsection{Passivity}
Passivity follows directly from the port-Hamiltonian formulation\DIFdelbegin \DIFdel{, the }\DIFdelend \DIFaddbegin \DIFadd{.
}\begin{proof}
\DIFadd{The }\DIFaddend above representation is of the standard from (\ref{EQ:generalPHS}). The terms to be omitted due to the skew-symmetry of the structure matrix $J(x)$ are not given in the following energy balance
\begin{eqnarray}
\begin{aligned}
...
\DIFaddend \subsection{Constrained dynamics IPC}\label{SS:constrainedDIPC}
...
\end{cases}
\end{equation}
The model-based controllers designed in section \ref{S:modelbasedcontrol} simulate the energetic state of the full cooperative set-up. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Assuming perfect modelling, the }\DIFaddend Hamiltonian energy function of the controller \DIFdelbegin \DIFdel{directly resembles }\DIFdelend \DIFaddbegin \DIFadd{provides a value for }\DIFaddend the energy level of the real system. The control behaviour is adapted to \DIFdelbegin \DIFdel{shape the energetic state both }\DIFdelend \DIFaddbegin \DIFadd{limit the energy virtually stored }\DIFaddend in the controller\DIFdelbegin \DIFdel{and in the controlled set-up to maintain a safe state. }\DIFdelend \DIFaddbegin \DIFadd{. A limited amount of energy in the controller leads to a limited amount of energy in the real system. If the modelling is perfect, the energy levels are equal.
       }\DIFaddend 
...
\end{aligned}
\end{eqnarray}
Here $(f_\g{t},e_\g{t})$ is the flow-effort pair, corresponding to input and output respectively. \DIFdelbegin \DIFdel{Towards the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend application of the energy tank \DIFdelbegin \DIFdel{we consider }\DIFdelend \DIFaddbegin \DIFadd{is demonstrated with }\DIFaddend an example port-Hamiltonian system of the \DIFdelbegin \DIFdel{form
}\begin{eqnarray*}
\begin{aligned}
\dot{\f{x}} &\DIFdel{= }[\DIFdel{J(\f{x}) - R(\f{x})}] \DIFdel{\frac{\partial \mathcal{H}}{\partial \f{x}} + G(\f{x})\f{u} }\\
\DIFdel{\f{y} }&\DIFdel{= G^T(\f{x}) \frac{\partial \mathcal{H}}{\partial \f{x}}
\end{aligned}
}\end{eqnarray*}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{standard input-state-output form, given in equation \ref{EQ:generalPHS}.
}\DIFaddend This standard system is chosen to keep the derivation process simple and can be replaced with the controllers from section \ref{S:modelbasedcontrol}.
\DIFdelbegin \DIFdel{First we seek to re-route the }\DIFdelend \DIFaddbegin \DIFadd{The re-routing of the virtually }\DIFaddend dissipated energy into the \DIFdelbegin \DIFdel{energy tank , this }\DIFdelend \DIFaddbegin \DIFadd{tank }\DIFaddend is accomplished by choosing the tank input as
\DIFdelbegin \DIFdel{$f_\g{t} = \frac{1}{x_\g{t}}\frac{\partial^T \mathcal{H}}{\partial \f{x}}R(\f{x})\frac{\partial \mathcal{H}}{\partial \f{x}} + \tilde{f}_\g{t}$. With a }\DIFdelend \DIFaddbegin \begin{equation}
 \DIFadd{f_\g{t} = \frac{1}{x_\g{t}}\frac{\partial^T \mathcal{H}}{\partial \f{x}}D(\f{x})\frac{\partial \mathcal{H}}{\partial \f{x}} + \tilde{f}_\g{t}
}\end{equation}
\DIFadd{The term $\frac{\partial^T \mathcal{H}}{\partial \f{x}}D(\f{x})\frac{\partial \mathcal{H}}{\partial \f{x}}$ is the dissipated power. A }\DIFaddend new input $\tilde{f}_\g{t}$ \DIFdelbegin \DIFdel{we have the }\DIFdelend \DIFaddbegin \DIFadd{is defined to preserve the power port for interconnection with the controller.  
With the new input the }\DIFaddend tank's energy balance \DIFaddbegin \DIFadd{becomes
...
\end{equation}
This corresponds to the systems dissipated energy plus some external supply. Next \DIFdelbegin \DIFdel{we use }\DIFdelend this external supply \DIFaddbegin \DIFadd{is used }\DIFaddend to interconnect tank and \DIFaddbegin \DIFadd{the standard }\DIFaddend port-Hamiltonian system\DIFaddbegin \DIFadd{, representing the controller}\DIFaddend . A power-preserving interconnection is established, \DIFdelbegin \DIFdel{here we use }\DIFdelend \DIFaddbegin \DIFadd{which is }\DIFaddend a combination of gyrator and transformer with ratio \DIFdelbegin \DIFdel{$n$
}\DIFdelend \DIFaddbegin \DIFadd{$\f{n}$. This type of interconnection allows the operator to control the power flow, despite being energetically decoupled. 
}\DIFaddend \begin{eqnarray}
\begin{aligned}
\f{u} &= \f{n}e_\g{t} \\
...
\end{equation}
\DIFdelbegin \DIFdel{The combined Hamiltonian energy function of the interconnected system is $\bar{\mathcal{H}}(\f{x},x_\g{t})=\mathcal{H}(\f{x})+\mathcal{T}(x_\g{t})$}\DIFdelend \DIFaddbegin \DIFadd{i.e. the transformer/gyrator is lossless for any $\f{n}$}\DIFaddend .
...
\begin{equation}\DIFaddbegin \label{EQ:TankInterconnection}
\DIFaddend \dot{\f{x}} = [J(\f{x}) - \DIFdelbegin \DIFdel{R}\DIFdelend \DIFaddbegin \DIFadd{D}\DIFaddend (\f{x})] \frac{\partial \bar{\mathcal{H}}}{\partial \f{x}} + G(\f{x})\frac{\f{w}}{x_\g{t}}e_\g{t} = [J(\f{x}) - \DIFdelbegin \DIFdel{R}\DIFdelend \DIFaddbegin \DIFadd{D}\DIFaddend (\f{x})] \frac{\partial \bar{\mathcal{H}}}{\partial \f{x}} + G(\f{x})\f{w}
\end{equation}
\DIFdelbegin \DIFdel{For a $x_\g{t}\rightarrow0$}\DIFdelend \DIFaddbegin \DIFadd{The combined Hamiltonian energy function of the interconnected system is $\bar{\mathcal{H}}(\f{x},x_\g{t})=\mathcal{H}(\f{x})+\mathcal{T}(x_\g{t})$.
An empty energy tank}\DIFaddend , i.e. \DIFdelbegin \DIFdel{an empty energy tank, there is a singularity}\DIFdelend \DIFaddbegin \DIFadd{$x_t=0$, results in a division by zero in equation (\ref{EQ:TankInterconnection})}\DIFaddend . Thus an complete depletion of the tank must be avoided. A minimum strategy is to introduce a switching parameter $\alpha$\DIFdelbegin \DIFdel{, we set }\DIFdelend \DIFaddbegin \DIFadd{. The transformer ratio is extended to }\DIFaddend $\f{n}=\frac{\alpha \f{w}}{x_\g{t}}$ with 
\begin{equation}
\alpha = \begin{cases}
...
\end{cases}
\end{equation}
This means that a control input is executed unchanged as long as certain amount $\epsilon$ of energy is in the tank. Once the \DIFdelbegin \DIFdel{energy budget is exceeded and }\DIFdelend \DIFaddbegin \DIFadd{tank is almost empty, i.e. $\mathcal{T}(x_\g{t}) < \epsilon$, further }\DIFaddend command execution possibly violates passivity\DIFdelbegin \DIFdel{the input is set }\DIFdelend \DIFaddbegin \DIFadd{. A negative energy level of the (virtual) tank has no physical meaning and compromises passivity by generating energy. The switching parameter $\alpha$ sets the input }\DIFaddend to $0$, \DIFaddbegin \DIFadd{thus }\DIFaddend effectively suspending energy exchange. This happens in both ways, neither is it possibly to re-transfer \DIFaddbegin \DIFadd{energy }\DIFaddend into the tank through the interconnection. At this point the tank can only be refilled by dissipation. Dissipation is associated with kinetic energy, \DIFaddbegin \DIFadd{i.e. }\DIFaddend as long as the system is moving it can recover from the input-suspended state. If the system is driven to a state of exclusively potential energy, there is no dissipation \DIFdelbegin \DIFdel{and a deadlock is reached. }\DIFdelend \DIFaddbegin \DIFadd{to refill the tank. If the tank is emptied below the threshold level, the input is ultimately suspended. The system cannot recover anymore because there is no kinetic energy to be dissipated and the switching parameter prevents the energy exchange with the controller.}\DIFaddend \\
\DIFdelbegin \DIFdel{Reaching a state of exclusively potential energy takes infinite time, this is because a state of higher potential energy can only be reached by motion. I. e. kinetic energy is a predecessor of potential energy. A fragment of the kinetic energy is dissipated and is available in the energy budget again. The available energy approaches $\epsilon$ asymptotically.}\DIFdelend \DIFaddbegin \DIFadd{The controllers designed in section \ref{S:modelbasedcontrol} store potential energy for two reasons, one is due to gravitation, the other is for deviations of desired and actual position. Gravitation is the more unproblematic case, elevating the system means to move the system. Moving means dissipating energy. In the elevating process a snmall portion of energy is dissipated and re-supplied to the tank. When getting close to the tank threshold, the available energy to be transformed into kinetic and then into potential energy gets asymptotically smaller. Only the previously dissipated energy is available and the elevation slows down and approaches to zero speed. Thus reaching a state of exclusively gravitational potential energy takes infinite time.
Deviations of actual and desired position result in potential energy stored in the springs connecting operator-object and object-manipulators. Consider now the system driven into an obstacle and the operator persisting to command motion in the blocked direction. The operator sets the desired object position and thus the springs are elongated. There is no dissipation because the system is blocked. If the dislocation consumes all the available energy, the system is deadlocked, i.e. the systems inputs are suspended permanently.}\DIFaddend \\  
...
\end{cases}
\end{equation}
\DIFdelbegin \DIFdel{Here }\DIFdelend \DIFaddbegin \DIFadd{where }\DIFaddend $\mathcal{T}_{\g{th}} > \epsilon$ is a threshold level that defines the width of the transition region. Adapting the reference trajectory leads to a permanent position deviation, if the user does not actively readjust after the tank level has recovered. \DIFdelbegin \DIFdel{We approach this problem in the next subsection }\DIFdelend \DIFaddbegin \DIFadd{The next subsection addresses this problem}\DIFaddend .\\ 
Regardless of a particular choice of $\alpha$\DIFdelbegin \DIFdel{we can give }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend a combined port-Hamiltonian representation of the system and the tank \DIFaddbegin \DIFadd{is as follows
}\DIFaddend \begin{eqnarray}
\begin{aligned}
\begin{pmatrix}
\dot{\f{x}} \\ \dot{x}_\g{t}
...
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\small
%DIFDELCMD < 	\def\svgwidth{0.95\columnwidth}
%DIFDELCMD < 	\input{energytank.eps_tex}
%DIFDELCMD < 	\vspace{10pt}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{The human operator controls the energy flow between tank and robotic system}}
	%DIFAUXCMD
%DIFDELCMD < \label{FIG:energytank}
%DIFDELCMD < \end{figure} 
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Note that there is no more an input-output port\DIFdelbegin \DIFdel{, this is because }\DIFdelend \DIFaddbegin \DIFadd{. This is because the external }\DIFaddend ports are defined by energy exchange of the system with its environment. It is shown in subsection \ref{SS:PHSinterconnection} that \DIFdelbegin \DIFdel{the systems}\DIFdelend \DIFaddbegin \DIFadd{systems, }\DIFaddend consistent with (\ref{EQ:generalPHS})\DIFdelbegin \DIFdel{are passive, analogously we show that }\DIFdelend \DIFaddbegin \DIFadd{, are passive. Analogously }\DIFaddend the described combination of such a system and the energy tank is \emph{lossless}\DIFdelbegin \DIFdel{.
...
\end{equation}
This equation is valid if the tank level $\mathcal{T}(x_\g{t})$ is above a certain threshold level $\mathcal{T}_{\g{th}}$. The \DIFdelbegin \DIFdel{spring's stiffness }\DIFdelend \DIFaddbegin \DIFadd{stiffness of the spring }\DIFaddend $k_{vb} \in \mathbb{R}^+$ is \DIFdelbegin \DIFdel{unaltered}\DIFdelend \DIFaddbegin \DIFadd{constant}\DIFaddend , $x_v, x_b \in \mathbb{R}$ denote the reference and the actual position respectively. Below the threshold \DIFdelbegin \DIFdel{we propose }\DIFdelend the following spring function \DIFaddbegin \DIFadd{is proposed
}\DIFaddend \begin{equation}
...
The two controllers presented in section \ref{S:modelbasedcontrol} are based on the model of a cooperative manipulation set-up. In an ideal case the same amount of energy stored in the energy is virtually bound in the controller\DIFdelbegin \DIFdel{. I.}\DIFdelend \DIFaddbegin \DIFadd{, i.}\DIFaddend e. the controller reflects the energy state of the real system. Since all energy sourced by the tank flows through the controller into the robots, bounding the energy in the controller also limits the energy stored in the robotic system.
...
The power exchanged between controller and the $i$-th robot is defined by the \DIFdelbegin \DIFdel{given by the }\DIFdelend wrench-twist product \DIFdelbegin \DIFdel{${W_i^{0,0}}^T T_i^0$}\DIFdelend \DIFaddbegin \DIFadd{$(W_i^{0,0})^T T_i^0$}\DIFaddend , applying the automatic compensation the energy balance is
\begin{equation}
\dot{\mathcal{T}}(x_\g{t}) + \DIFdelbegin %DIFDELCMD < \underbrace{\sum_{i=1}^n {W_i^{0,0}}^T T_i^0}%%%
\DIFdelend \DIFaddbegin \underbrace{\sum_{i=1}^n (W_i^{0,0})^T T_i^0}\DIFaddend _{\text{compensation}} = - \dot{{\mathcal{H}}}(x) + \sum_{i=1}^n \DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend W_i^{0,0}\DIFdelbegin %DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{)}\DIFaddend ^T T_i^0
\end{equation}
This means a constant level of energy in tank and controller $\dot{\mathcal{T}}(x_\g{t})+\dot{\mathcal{H}} = 0$.
Clearly, the compensation term \DIFdelbegin \DIFdel{$\sum_{i=1}^n {W_i^{0,0}}^T T_i^0$ is active, it }\DIFdelend \DIFaddbegin \DIFadd{$\sum_{i=1}^n (W_i^{0,0})^T T_i^0$ is active. It }\DIFaddend provides energy in lieu of the user, who is energetically decoupled in our approach. Still all energy flows through the controller and is bounded if necessary. But the energy budget, given by the tank level, accounts only for the controller state. Please recall that the combination of controller and tank is lossless, so the system can always carry out tasks within the given safety margin.\\ 
...
Simulations of the proposed control schemes and existing approaches from literature are conducted to evaluate and compare their effectiveness. The controllers for cooperative manipulation are compared in terms of trajectory tracking \DIFdelbegin \DIFdel{, dynamic behaviour }\DIFdelend and internal stress on the object. To preserve comparability, they are adjusted to exert equal \DIFdelbegin \DIFdel{strength of }\DIFdelend force and the tuning parameters are kept within the same range.\\
In the simulation the robots \DIFdelbegin \DIFdel{we assume the robots }\DIFdelend \DIFaddbegin \DIFadd{are assumed }\DIFaddend to behave like simple inertias, i.e. their underlying dynamics \DIFaddbegin \DIFadd{(configuration dependent inertia, Coriolis and gravity terms) }\DIFaddend is compensated. The connection between common object (another inertia) and the end-effectors is considered rigid. For the computation of the combined dynamics \DIFdelbegin \DIFdel{we apply }\DIFdelend the principle of \DIFdelbegin \DIFdel{constrained motion , }\DIFdelend \DIFaddbegin \DIFadd{least constraint motion is applied, which is }\DIFaddend explicitly given for a cooperative manipulation set-up in \cite{Erhart_16}. The same article provides the routine used for the calculation of the stress exerted on the object (internal wrench).
Simulations are conducted in \emph{MATLAB/Simulink}, using a fixed step solver with a step size of $1$ms\DIFdelbegin \DIFdel{, this }\DIFdelend \DIFaddbegin \DIFadd{. This }\DIFaddend facilitates application in an experimental set-up (the control units typically run at $1$kHz). \DIFaddbegin \DIFadd{The }\DIFaddend Parameters used in the simulations can be found in table \ref{TAB:generalsimulationprop}. The control input is a  reference velocity, the set-point position/orientation is computed by integration. Fig. \ref{FIG:ReferenceTrajectory} shows the two test cases, linear (along the x-axis) and angular velocity (around the z-axis). 
\begin{table}[h]
	\centering
...
%DIF < \begin{figure}
%DIF < \include{DePasrot}
%DIF < \caption[Internal force impedance control with feed-forward of the object dynamics: Rotation]{Internal force impedance control with feed-forward of the object dynamics: Rotation; Graphs from top: Orientation (desired/actual), Angular Velocity (desired/actual), Force exerted by one manipulator, Internal wrench}
%DIF < \end{figure}
\DIFaddbegin \DIFadd{In literature several control schemes for cooperative manipulation are known. The focus is limited to those that achieve stability during contact and non-contact transitions. Those include impedance control and Intrinsically Passive Control (IPC). Simulation results are provided for two impedance control schemes and one IPC scheme. The dynamic performance is discussed with regard to the different role of inertias in either control paradigms.
}\DIFaddend 

\subsection{Internal and external impedance based reference trajectory generation}\DIFaddbegin \label{SS:CaVi}
\DIFaddend A renowned approach by Caccavale and Villani \cite{Caccavale_01, Caccavale_08} consists of a cascaded architecture enforcing two impedance relations. One is established between the given reference trajectory and the object, \DIFdelbegin \DIFdel{we refer to an }\DIFdelend \DIFaddbegin \DIFadd{termed as the }\DIFaddend \emph{external} impedance. The other is between the manipulators and the object and is called \DIFaddbegin \DIFadd{the }\DIFaddend \emph{internal} \DIFaddbegin \DIFadd{impedance}\DIFaddend . The output of the impedance loops is a compliant reference trajectory which is used in an \emph{inner motion control} loop to generate forces to drive the robot. The parameters used in the simulation are chosen very close to the ones given in \cite{Caccavale_08}, see table \ref{TAB:CaViParameters}. 
...
The simulation results are presented in figure \ref{FIG:CaViSim}, the left column shows the linear motion test case and the right the rotational one. The plots on top show the deviation from the desired position and orientation respectively. \DIFdelbegin \DIFdel{Below the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend velocity errors are displayed \DIFaddbegin \DIFadd{below}\DIFaddend . The third plot row shows the wrenches applied by one of the manipulators. The bottom row indicates the internal stress a manipulator exerts on the object.\\
The approach shows good trajectory tracking\DIFdelbegin \DIFdel{, especially the }\DIFdelend \DIFaddbegin \DIFadd{. The }\DIFaddend velocity error is the lowest in this comparative study. The internal wrench is low and is similar to the other approaches\DIFdelbegin \DIFdel{, for linear motion not any internal stress occurs. }\DIFdelend \DIFaddbegin \DIFadd{. For pure linear motion the internal stress is approx. zero.
}\DIFaddend 
...
\begin{figure}[h]
\input{CaVigroup}
\DIFdelbeginFL %DIFDELCMD < \label{FIG:CaViSim}
%DIFDELCMD < %%%
\DIFdelendFL \caption[Simulation results: Internal and external impedance based reference trajectory generation]{Internal and external impedance based reference trajectory generation: Translation (left column) and rotation}
\DIFaddbeginFL \label{FIG:CaViSim}
\DIFaddendFL \end{figure}
...
Erhart and Hirche \cite{Erhart_16} omit the external impedance relation\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend but include object dynamics feed-forward\DIFdelbegin \DIFdel{. I.}\DIFdelend \DIFaddbegin \DIFadd{, i.}\DIFaddend e. the wrench necessary to accelerate the object's inertia is distributed by an inverse of the grasp matrix to the manipulators. This results in even better position/orientation accuracy compared to the \DIFdelbegin \DIFdel{previous}\DIFdelend \DIFaddbegin \DIFadd{approach presented in subsection \ref{SS:CaVi}}\DIFaddend . However results presented in \cite{Caccavale_08} indicate that, in absence of an external impedance relation, higher forces occur, if the object comes into contact with the environment.\\
Interestingly the magnitude of the internal forces (during rotation) depends directly on the step size of the simulation (running a fixed step solver), i.e. a ten-times smaller step size gives ten-times smaller internal forces. Internal forces are calculated based on the geometry in the last simulation step. The correlation between step size and values indicates that these forces are rather \DIFdelbegin \DIFdel{due to }\DIFdelend \DIFaddbegin \DIFadd{a consequence of }\DIFaddend the discrete nature of the simulation than of the control law generating internal stress.\\
...
\begin{figure}[h]
\input{Depasgroup}
\DIFdelbeginFL %DIFDELCMD < \label{FIG:DePaSim}
%DIFDELCMD < %%%
\DIFdelendFL \caption[Simulation results: Internal impedance control with feed-forward of the object dynamics]{Internal impedance control with feed-forward of the object dynamics: Translation (left column) and rotation}
\DIFaddbeginFL \label{FIG:DePaSim}
\DIFaddendFL \end{figure}
...
\subsection{\DIFdelbegin \DIFdel{Classic }\DIFdelend \DIFaddbegin \DIFadd{Dynamic }\DIFaddend IPC\DIFdelbegin \DIFdel{by Stramigioli}\DIFdelend }
The IPC \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend introduced by Stramigioli \cite{Stramigioli_01} and experimentally evaluated by Wimb\"ock et al. \cite{Wimboeck_08} on a robotic hand. \DIFdelbegin \DIFdel{In Stramigioli 's version }\DIFdelend \DIFaddbegin \DIFadd{Stramigioli applies pure stiffness control, i.e. }\DIFaddend there are no dampers along the \DIFdelbegin \DIFdel{manipulator-object springsand Wimboeck uses only translational}\DIFdelend springs. \DIFaddbegin \DIFadd{Wimb\"ock et al. add the dampers to enhance the trajectory tracking behaviour and call the scheme }\emph{\DIFadd{dynamic IPC}}\DIFadd{. See also figure \ref{FIG:ImpedanceIPC} for a comparison. The implementation of Wimb\"ock is purely translational, i.e. forces but no moments are transferred via the springs and the dampers. }\DIFaddend The simulation results presented here\DIFaddbegin \DIFadd{, called dynamic IPC, }\DIFaddend are a combination of both \DIFdelbegin \DIFdel{implementations, we use dampers along }\DIFdelend \DIFaddbegin \DIFadd{approaches, dampers along the }\DIFaddend 6-DoF springs \DIFdelbegin \DIFdel{. the architecture has been detailed throughout this work.}\DIFdelend \DIFaddbegin \DIFadd{are used.}\DIFaddend \\
The impedance parameters are chosen equal or lower for translation and significantly higher for rotation, see table \ref{TAB:IPCParameters}.\DIFaddbegin \\ \DIFadd{The feasible stiffness and damping in this implementation is limited by numerical stability, a further increase leads to infinite values at some point of the simulation. This issue also requires the use of }\emph{\DIFadd{Simulink's ode8}} \DIFadd{solver for this approach, instead of }\emph{\DIFadd{ode1}} \DIFadd{(Euler's method) used for the other simulations. The }\emph{\DIFadd{ode8}} \DIFadd{method is of the }\emph{\DIFadd{Dormand-Prince}} \DIFadd{class with an accuracy order of $8$. The rate of rate of convergence improves at the cost of higher computational complexity \mbox{%DIFAUXCMD
\cite{SimulinkHelp:Solver}}%DIFAUXCMD
.}\\
 \DIFaddend The results (fig. \ref{FIG:IPCSim}) show a clearly inferior trajectory tracking compared to the previous approaches.
\DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Discussion}}
 \DIFadd{The difference of dynamic performance of impedance control and IPC }\DIFaddend stems from a different mode of function of the inertias in the IPC. In classic impedance control clearly the inertia is in parallel with the spring and in the IPC the inertia is connected in series, figure \ref{FIG:ImpedanceIPC} illustrates the concepts. In the first case the inertia is used to calculate the wrench necessary to accelerate the associated body, in the latter the spring \DIFdelbegin \DIFdel{and damper }\DIFdelend \DIFaddbegin \DIFadd{(and damper) }\DIFaddend need to provide the wrench to accelerate the additional (virtual) body. Thus the inertia in the IPC is an energy storing element that reduces the controller dynamics. On the other hand the IPC does not require absolutely smooth reference velocities. The classic impedance control approaches use the time-derivative of the input velocity and often require pretreatment (e.g. low-pass filtering) of the input.\\
\begin{figure}[H]
	\centering
	\small
	\def\svgwidth{0.95\columnwidth}
	\DIFdelbeginFL %DIFDELCMD < \input{ImpedanceIPC.eps_tex}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \input{ImpedancedIPC.eps_tex}
	\DIFaddendFL \caption{Arrangement of spring, mass and damper in classical impedance control\DIFdelbeginFL \DIFdelFL{approaches }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, dynamic \mbox{%DIFAUXCMD
\cite{Wimboeck_08} }%DIFAUXCMD
}\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{in the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{classic }\DIFaddendFL IPC \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\cite{Stramigioli_01} }%DIFAUXCMD
}\DIFaddendFL }
	\label{FIG:ImpedanceIPC}
\end{figure}
\DIFdelbegin \DIFdel{The feasible stiffness and damping in this implementation is limited by numerical stability, a further increase leads to infinite values at some point of the simulation. This issue also requires the use of }\emph{\DIFdel{Simulink's ode8}} %DIFAUXCMD
\DIFdel{solver for this approach, instead of }\emph{\DIFdel{ode1}} %DIFAUXCMD
\DIFdel{(Euler's method) used for the other simulations. The }\emph{\DIFdel{ode8}} %DIFAUXCMD
\DIFdel{method is of the }\emph{\DIFdel{Dormand-Prince}} %DIFAUXCMD
\DIFdel{class with an accuracy order of $8$. The rate of rate of convergence improves at the cost of higher computational complexity \mbox{%DIFAUXCMD
\cite{SimulinkHelp:Solver}}%DIFAUXCMD
. 
}\DIFdelend 
...
\begin{figure}
\input{StrIPCgroup}
\DIFaddbeginFL \caption[Simulation results: Dynamic IPC]{\DIFaddFL{Dynamic IPC: Translation (left column) and rotation}}
\DIFaddendFL \label{FIG:IPCSim}
\DIFdelbeginFL %DIFDELCMD < \caption[Simulation results: Intrinsically Passive Controller]{%
{%DIFAUXCMD
\DIFdelFL{Intrinsically Passive Controller: Translation (left column) and rotation}}
%DIFAUXCMD
...
\begin{figure}
\input{DIPCcgroup}
\DIFdelbeginFL %DIFDELCMD < \label{FIG:DIPCcSim}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL 

\DIFaddendFL \caption[Simulation results of the constrained dynamic IPC]{Constrained dynamic IPC: Translation (left column) and rotation}
\DIFaddbeginFL \label{FIG:DIPCcSim}
\DIFaddendFL \end{figure}

\section{Compliant trajectory generating IPC}
The compliant trajectory generating IPC uses an force control loop to compute the force commands for the robots. The structure can either be viewed as PI-control or as a spring and a damper in parallel. Gains or stiffness/damping can be chosen high because the superimposed control scheme already establishes compliance \cite{Caccavale_08}\DIFdelbegin \DIFdel{, the }\DIFdelend \DIFaddbegin \DIFadd{. The }\DIFaddend simulation settings are summarized in table \ref{TAB:DIPCrParameters}. Numerical stability problems also occur with this approach, \DIFdelbegin \DIFdel{the reported parameters the maximum feasible }\DIFdelend \DIFaddbegin \DIFadd{damping and stiffness cannot be chosen as high as for in the previous simulations. The parameters reported in table \ref{TAB:DIPCrParameters} are chosen as high as possible to achieve numerical stability }\DIFaddend with a fixed step solver (\emph{ode8}) at $1$ ms step size. \DIFdelbegin \DIFdel{With a }\DIFdelend \DIFaddbegin \DIFadd{A }\DIFaddend variable step solver (e.g. \emph{ode45}) \DIFaddbegin \DIFadd{is stable with }\DIFaddend higher stiffness and damping \DIFdelbegin \DIFdel{are reachable }\DIFdelend and performance is superior. Due to non-applicability in real robotic system, \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{variable-step }\DIFaddend solvers are not considered further. \DIFdelbegin \DIFdel{Stiffness }\DIFdelend \DIFaddbegin \DIFadd{Although the chosen stiffness }\DIFaddend and damping is \DIFdelbegin \DIFdel{similar }\DIFdelend \DIFaddbegin \DIFadd{still comparable }\DIFaddend to the classic IPC parameters, the dynamic performance is inferior because of the additional inertias, which have to be accelerated. This results in approx. double errors in position/orientation and velocity compared to the classic IPC \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend see figure \ref{FIG:DIPCrSim}\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{). }\DIFaddend In contrast to the constrained dynamic IPC the virtual inertias for the object and the manipulators cannot be chosen smaller than their real pendants. Doing so leads to infinite simulation results. This entails in an even wider performance gap to the constrained dynamic IPC.
...
\begin{figure}
\input{DIPCrgroup}
\DIFdelbeginFL %DIFDELCMD < \label{FIG:DIPCrSim}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL 

\DIFaddendFL \caption[Simulation results of the compliant reference trajectory generating IPC]{Compliant reference trajectory generating IPC: Translation (left column) and rotation}
\DIFaddbeginFL \label{FIG:DIPCrSim}
\DIFaddendFL \end{figure}

\clearpage
\section{Energy-bounded trajectory tracking}
In the presented simulation, trajectory tracking demands more energy than is available from the energy tank (detailed in section \ref{S:EnergyTanks}). The budget for performing operations is limited to $30$ J, in accordance with the safety limit of neck fractures (see section \ref{S:PassivitySafety}). Trajectories exceeding this limit are adapted with \DIFdelbegin \DIFdel{a }\DIFdelend the variable stiffness and damping technique presented in subsection \ref{SS:EnergyAdaptedStiffness}. The underlying control scheme is the dynamic constrained IPC from section \ref{S:modelbasedcontrol}.\\
In the simulation \DIFdelbegin \DIFdel{we assume that }\DIFdelend no energy is lost in the robotic system (due to friction or work on the environment). If the user commands acceleration, energy is transferred from the tank through the controller into the robotic system and is thereby stored in form of potential and energy. When the system slows down, the kinetic and potential energy is is re-transformed into free energy and stored in the tank. This happens in an ideal, lossless way, thus the energy tank reaches the former level after an operation \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend see the third plot in figure \ref{FIG:VarStiff}\DIFaddbegin \DIFadd{)}\DIFaddend . Upon depletion of the tank (approx. at $t=2$ s), the kinetic energy cannot be further increased  and the velocity stays constant (observable from the second plot). The reduction of \DIFdelbegin \DIFdel{stiffness and }\DIFdelend \DIFaddbegin \DIFadd{the stiffness and the }\DIFaddend damping starts as soon as the level falls under a certain threshold, here $T_{th}=10$ J. As a consequence from $t=1.7$ s on the velocity increases slower. The actual position falls behind the desired (first plot in figure \ref{FIG:VarStiff})\DIFdelbegin \DIFdel{, with }\DIFdelend \DIFaddbegin \DIFadd{. With }\DIFaddend the reduced velocity it takes until $t=2.6$ s to catch up to the desired position. During this time the velocity is kept at the energetically feasible maximum. As soon as the velocity reduces the energy level recovers quickly. The approach attempts optimal position tracking under the given energy constraint and no permanent deviations occur after the energetic level has recovered.
\begin{figure}
\input{VarStifftrans}
\DIFdelbeginFL %DIFDELCMD < \label{FIG:VarStiff}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL 

\DIFaddendFL \caption[Simulation results of the energy bounding by a variable stiffness spring]{Energy bounding by a variable stiffness spring and a variable damper}
\DIFaddbeginFL \label{FIG:VarStiff}
\DIFaddendFL \end{figure}
...
\DIFaddend Cooperative manipulation is analysed for the first time in the port-Hamiltonian setting. \DIFdelbegin \DIFdel{We provide a }\DIFdelend \DIFaddbegin \DIFadd{The port-Hamiltonian formulation allows to design a model-based controller by interconnecting atomic mechanical elements and facilitates passivity-based control. It is shown how the modelling of a cooperative set-up transfers into a virtual structure to control the formation of robots. In modelling, a special focus is on the rigid connection of manipulators and object. This constrained contact situation is common in cooperative manipulation. In contrast to \mbox{%DIFAUXCMD
\cite{Erhart_16}}%DIFAUXCMD
, this thesis provides a }\DIFaddend self-contained \DIFdelbegin \DIFdel{derivation of the constraint forces arising when the manipulators are rigidly connected to the common object. We show }\DIFdelend \DIFaddbegin \DIFadd{derivation of the internal forces that arise from the constraints. It is shown }\DIFaddend that these forces do not generate, store or dissipate energy, i.e. \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend rigid connection is energy consistent. The modelling of the rigid connections is also used for control.\\
Two model-based controllers are designed for the teleoperated, shared control task of cooperative object manipulation. \DIFdelbegin \DIFdel{Their main characteristic is the virtual structure that resembles an impedance-controlled cooperative manipulation set-up}\DIFdelend \DIFaddbegin \DIFadd{They use a virtual structure built from springs, masses and dampers to achieve a desired group behaviour of the robots}\DIFaddend . The design is based on physical intuition and is intrinsically passive. \DIFdelbegin %DIFDELCMD < \\ %%%
\DIFdel{Simulation results show that the dynamic response of the proposed controllers is inferior by one order of magnitude, compared to classic cooperative manipulation approaches. Stramigioli's IPC has a similar architecture and the performance compares well to our controllers.
Classic cooperative manipulation approaches use the desired acceleration to calculate the forces necessary to speed up the inertias in the set-up.The proposed controllers and the IPC do not }\DIFdelend \DIFaddbegin \DIFadd{The two proposed controllers differ in terms of their output variables, one generates a compliant reference trajectory for each robot and the other issues force commands to the robots.
...
\DIFadd{The proposed control design is validated by simulative comparison against cooperative manipulation control schemes known from literature. Impedance control approaches show better dynamic behaviour but are not passive, i.e. achieve stability with a more limited class of environments. The dynamic IPC by Wimb\"ock et al. \mbox{%DIFAUXCMD
\cite{Wimboeck_08} }%DIFAUXCMD
is passive and has an architecture similar to the proposed controllers. To allow for a comparison with the proposed approaches,  it is extended to six DoF and simulated. The results compare well. Internal forces indicate how well a controller preserves the formation of the robots. All simulated approaches show internal forces close to zero}\DIFaddend , \DIFdelbegin \DIFdel{plus have additional virtual inertias that retard dynamic motion.In the simulations the classic IPC and the reference trajectory generating controller suffer from numeric instabilities, the constrained dynamics IPC does not and is therefore preferred for safety concerns. }%DIFDELCMD < \\      
%DIFDELCMD < %%%
\DIFdel{Input interfaces, that do not impose a counteracting force or have deflection barriers, allow a human to issue arbitrary commands,  which may be unstable and /or unsafe. The model-based control design on energetic level calculates the energetic state of the controller, which directly reflects the situation in the real set-up. This allows to distinguish between energy appropriately spent on the task and energy driving the system to a potentially harmful state. The latter type is kept bounded by energy shaping control, }\DIFdelend i.e. \DIFdelbegin \DIFdel{dynamically adapting the controller parameters}\DIFdelend \DIFaddbegin \DIFadd{good coordination performance}\DIFaddend .\\
The \DIFdelbegin \DIFdel{controlled robotic system is sourced from an energy tank, the energy supplied to the robots is re-fed into the tank, }\DIFdelend \DIFaddbegin \DIFadd{proposed schemes are designed for shared control, }\DIFaddend i.e. \DIFdelbegin \DIFdel{the controller energy level is bounded. Through the automatic re-filling an infinite amount of energy may be supplied over time. Then the robots do not reach an energetic minimum. Thus we cannot conclude asymptotic stability , which follows from a passive robotic system with a finite supply of energy. However by bounding the }\DIFdelend \DIFaddbegin \DIFadd{involve the human in the control loop. Humans are commonly considered fail-safe in teleoperation. This is the first work to consider stability and safety issues with non-reactive interfaces. The human is virtually coupled by a spring and a damper to the system. The model-based control design on energy level allows to identify potentially harmful energy in the system. The controller relaxes the coupling of the human, if an unsafe state is detected. The adaptive coupling then prevents the human from supplying further energy to the system. An energy tank is used to passivate the coupling.}\\
\DIFadd{Asymptotic stability is ensured by the passive controller for finite }\DIFaddend energy \DIFdelbegin \DIFdel{in the controller the energy in the real set-up gets also limited and the system is at a stable energetic state.This is valid if the environment provides only a finite amount of energy .}\DIFdelend \DIFaddbegin \DIFadd{supplies. For human commands that require infinite energy for execution, the energy-bounding still achieves stability.}\DIFaddend \\
Limiting the energy \DIFdelbegin \DIFdel{, }\DIFdelend also restricts the possible impact in case of a collision with a human or the environment \DIFaddbegin \DIFadd{and thus enhances safety}\DIFaddend . The prime task in cooperative manipulation is the transportation of large and/or heavy objects, usually this is carried out at low speeds. Injuries due to violent pressure on a clamped human may be more relevant in this set-up. The proposed energy shaping approach also limits the possible force, but the used safety metrics should be analysed especially for this case\DIFaddbegin \DIFadd{. Another future extension of this work is to consider direct human-robot team interaction under the leader-follower paradigm}\DIFaddend .  
