...
variablename = [Position,desiredPosition,Velocity,desiredVelocity,ManipulatorWrench,InternalWrench];
...
if w==2
    ...
    copyfile(sprintf('%s_%s.txt',modelname,variablename(k).name),'../../Latex/plotdata/');
else
    dlmwrite(sprintf('%s_%s_tx.txt',modelname,variablename(k).name),matr(:,[1,2]),'\t');
    copyfile(sprintf('%s_%s_tx.txt',modelname,variablename(k).name),'../../Latex/plotdata/');
    dlmwrite(sprintf('%s_%s_ty.txt',modelname,variablename(k).name),matr(:,[1,3]),'\t');
    copyfile(sprintf('%s_%s_ty.txt',modelname,variablename(k).name),'../../Latex/plotdata/');
    dlmwrite(sprintf('%s_%s_rz.txt',modelname,variablename(k).name),matr(:,[1,7]),'\t');
    copyfile(sprintf('%s_%s_rz.txt',modelname,variablename(k).name),'../../Latex/plotdata/');
end
end
