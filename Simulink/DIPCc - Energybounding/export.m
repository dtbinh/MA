...
modelname = 'VarStifftrans';
variablename = [Position,desiredPosition,Velocity,desiredVelocity,TankLevel,ManipulatorWrench];
...
yaxistext = {'$\Phi_z$ [rad]','$\omega_z$ [rad/s]','$\Phi_z$ [rad]','$\omega_z$ [rad/s]','$f$[N],$m$[Nm]','$f$[N],$m$[Nm]'};
legendtext = {'error orientation $\Delta \Phi_z$','error angular velocity $\Delta \omega_z$','error orientation $\Delta \Phi_z$','error angular velocity $\Delta \omega_z$','force $f_x$, force $f_y$, torque $m_z$','force $f_{int,x}$, force $f_{int,y}$, torque $m_{int,z}$'};
